...
const CategoryProductList = ({ categoryProductList }) => {
  ...
  return (
    ...
    // >
      <Box my="-0.25rem">
        <Carousel totalSlides={categoryProductList.length} visibleSlides={visibleSlides}>
                ...
                <Link href={`/product/${item.id}`}>
                  <a>
                    ...
                    <FlexBox gap={1}>
                      ...
                      ৳{Math.ceil(item.cash_price).toLocaleString()}
                      </H4>

                      <H4 fontWeight="600" fontSize="14px" color="grey.600">
                        <del>৳{Math.ceil(item.unit_price).toLocaleString()}</del>
                      </H4>
                    </FlexBox>
                  </a>
                </Link>
